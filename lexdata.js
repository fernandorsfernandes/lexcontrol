// ============================================================
// LEXDATA.JS — Banco de dados central do LexControl v2.0
// Corrigido: broadcast limpo, postMessage, seed consistente
// ============================================================
...
// ── BROADCAST CORRIGIDO ───────────────────────────────────
const broadcast = (key) => {
  // 1. Notifica outros iframes via postMessage no pai
  try {
    if (window.parent && window.parent !== window) {
      window.parent.postMessage({ type: 'lex-storage', key }, '*');
    }
  } catch(e){}
  // 2. Notifica o próprio documento (útil quando roda standalone)
  try {
    window.dispatchEvent(new CustomEvent('lex-storage', { detail: { key } }));
  } catch(e){}
  // 3. Notifica o pai diretamente se for iframe
  try {
    if (window.parent && window.parent !== window) {
      window.parent.dispatchEvent(new CustomEvent('lex-storage', { detail: { key } }));
    }
  } catch(e){}
};
...
const applyTema = (v) => {
  ...
  try {
    if (window.parent && window.parent !== window) {
      window.parent.postMessage({ type: 'lex-tema', tema: v || getTema() }, '*');
    }
  } catch(e){}
};
...
const marcarParcela = (honId, parcelaId, status) => {
  ...
  if (hon) {
    const p = hon.parcelas.find(p => p.id === parcelaId);
...
const seed = () => {
  ...
  clis.forEach(c => saveCliente(c));

  const evs = [
    {id:uid(),titulo:'Audiência — Maria Graças',tipo:'Audiência',data:'2026-05-10',hora:'14:00',local:'3ª Vara Cível',processo:'0012345-67.2024.8.09.0051',status:'Confirmado'},
    {id:uid(),titulo:'Prazo — Contestação XYZ',tipo:'Prazo',data:'2026-05-15',hora:'23:59',local:'',processo:'0031122-44.2025.8.09.0051',status:'Pendente'},
    {id:uid(),titulo:'Reunião Ana Rodrigues',tipo:'Reunião',data:today(),hora:'10:00',local:'Escritório',processo:'',status:'Confirmado'},
  ];
  evs.forEach(e => saveEvento(e));
...
(function(){
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => { LexData.seed(); LexData.initTema(); });
  } else {
    LexData.seed();
    LexData.initTema();
  }
